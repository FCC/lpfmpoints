...
(function($) {
  if (typeof(google.maps.CartoDBLayer) === "undefined") {
    ...
    google.maps.CartoDBLayer = function (params) {
      ...
      function autoBound(params) {
        ...
        if (!params.map_key) {
          $.ajax({
            url:'//'+params.user_name+'.cartodb.com/api/v2/sql/?q='+escape('select ST_Extent(the_geom) from '+ params.table_name),
            dataType: 'jsonp',
            timeout: 2000,
      ...
      function setCartoDBMapStyle(params) {
        $.ajax({
          url: '//' + params.user_name + '.cartodb.com/tiles/' + params.table_name + '/map_metadata?'+ 'map_key=' + (params.map_key || ''),
          dataType: 'jsonp',
          timeout: 2000,
          callbackParameter: 'callback',
          success: function(result) {
      ...
      function addSimpleCartoDBTiles(params) {
        ...
        var cartodb_layer = {
          getTileUrl: function(coord, zoom) {
            return '//' + params.user_name + '.cartodb.com/tiles/' + params.table_name + '/'+zoom+'/'+coord.x+'/'+coord.y+'.png?sql='+params.query;
          },
      ...
      function generateTileJson(params) {
        var core_url = '//' + params.user_name + '.cartodb.com';  
  ...
  CartoDB.Infowindow.prototype.open = function(feature,latlng){
    ...
    $.ajax({
      url:'//'+ this.params_.user_name +'.cartodb.com/api/v2/sql/?q='+infowindow_sql,
      dataType: 'jsonp',
      timeout: 2000,
      callbackParameter: 'callback',
      success: function(result) {
